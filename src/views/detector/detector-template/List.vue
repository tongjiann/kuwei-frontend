<script setup lang="ts" name="DetectorTemplate">
...
const { queryParam, loading, dataList, pagination, getList, onSearch, resetFilter, remove, batchRemove } =
  useList<DetectorTemplate>({ baseApi, initQueryParam: {} })

defineExpose({
  refresh: onSearch,
  addCondition: (key: keyof DetectorTemplate, value: any, refresh = false) => {
...
const handleOperation = (code: string, value?: string | string[], row?: DetectorTemplate) => {
  switch (code) {
    case 'detail':
...
const sum = (prop: keyof DetectorTemplate, fractionDigits?: number) =>
  Number(
    (dataList ?? [])
...
const ave = (prop: keyof DetectorTemplate, fractionDigits?: number) =>
  Number(
...
const activated = () => {
  nextTick(() => {
    ...
    for (const queryKey in query) {
      queryParam[queryKey as keyof DetectorTemplate] = query[queryKey] as any
    }
...
<template>
  <div class="list-page">
    ...
    <el-table
      ...
      @row-dblclick="(row: DetectorTemplate) => checkPermission('detail') && handleOperation('detail', row.id)"
      @selection-change="(arr: Array<DetectorTemplate>) => (selectedKeys = arr.map(e => `${e.id}`))"
    ...
    >
      ...
      <el-table-column label="参数数量" prop="parameterCount" show-overflow-tooltip>
        <template #header>
          <el-tooltip placement="bottom">
            <template #content>
              当前页总和：{{ sum('parameterCount', 2) }}<br />
              当前页平均：{{ ave('parameterCount', 3) }}
            </template>
            参数数量
          </el-tooltip>
        </template>
      ...
      <el-table-column label="操作" fixed="right" :width="180">
        <template #default="{ row }: { row: DetectorTemplate }">
          <el-space>
            <el-button
